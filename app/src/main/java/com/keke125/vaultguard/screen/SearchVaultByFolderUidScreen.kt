...
import com.keke125.vaultguard.model.SearchVaultByFolderUidViewModel
...
@Composable
fun SearchVaultByFolderUidScreen(
    navController: NavController,
    navigateToViewVault: (Int) -> Unit,
    navigateToEditVault: (Int) -> Unit,
    viewModel: SearchVaultByFolderUidViewModel = viewModel(factory = AppViewModelProvider.Factory)
) {
    VaultGuardTheme {
        Surface(
            modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background
        ) {
            val vaultUiState by viewModel.searchVaultByFolderUidUiState.collectAsState()
            ...
            Scaffold(topBar = {
                ...
                }, title = { Text(stringResource(SearchVaultByFolderUidDestination.titleRes)) })
            }) { innerPadding ->
                SearchBar(query = keyword,
                    onQueryChange = {
                        onKeywordChange(it)
